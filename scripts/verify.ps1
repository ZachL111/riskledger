...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-riskledger-detail.ps1

sqlite3 :memory: ".read tests/domain_review.sql"

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-riskledger-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-riskledger-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-riskledger-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
